# The public interface of this layer. The account-baseline layer reads these via
# terraform_remote_state, so account ids are never copied by hand.

output "ou_ids" {
  description = "OU name to id, for every OU this repository created. SCPs and baselines attach to these."
  value       = { for name, ou in aws_organizations_organizational_unit.this : name => ou.id }
}

output "ou_arns" {
  description = "OU name to ARN. Control Tower targets take ARNs, not ids — a difference that costs an hour the first time."
  value       = { for name, ou in aws_organizations_organizational_unit.this : name => ou.arn }
}

output "account_ids" {
  description = "Account name to id. The account-baseline layer iterates this to build a provider per account."
  value       = { for k, v in aws_organizations_account.this : k => v.id }
}

output "account_arns" {
  description = "Account name to ARN."
  value       = { for k, v in aws_organizations_account.this : k => v.arn }
}

output "account_environments" {
  description = "Account name to environment label, so the baseline layer can vary log retention and gating by environment."
  value       = { for k, v in local.register : k => v.environment }
}

output "cross_account_role_arns" {
  description = <<-EOT
    The role in each new account that the bootstrap pipeline assumes to configure it.

    Created automatically by AWS Organizations at account creation, trusting the management
    account. This is why the account-baseline layer needs no stored credentials for the
    accounts it configures.
  EOT
  value = {
    for k, v in aws_organizations_account.this :
    k => "arn:${local.partition}:iam::${v.id}:role/OrganizationAccountAccessRole"
  }
}

output "guardrail_policy_id" {
  description = "Our SCP. One policy holding five statements, because AWS caps policies-per-target at five and Control Tower already uses part of that budget."
  value       = aws_organizations_policy.workloads_guardrails.id
}

output "control_tower_enrolled" {
  description = "Whether OUs were registered with Control Tower and therefore inherit its 16 controls."
  value       = var.enroll_in_control_tower
}

output "account_register" {
  description = <<-EOT
    THE TRACKING LAYER. The register joined to real account ids — what this repository has
    actually provisioned.

    Printed to the job summary after every apply, so the run that creates an account also
    records what now exists.

    No email here: it is personal data and permanent, and it stays in SSM.

    Deliberately NOT written to a DynamoDB table or a committed JSON file. Either would be
    a third copy of the truth that can drift from the register and from state. The
    queryable index is the account TAGS, and this output is the human-readable join.
  EOT
  value = {
    for name, entry in local.register : name => {
      account_id  = aws_organizations_account.this[name].id
      arn         = aws_organizations_account.this[name].arn
      ou          = entry.ou
      environment = entry.environment
      project     = entry.project
      cost_centre = entry.cost_centre
      owner       = entry.owner
      requested   = entry.requested
      ticket      = try(entry.ticket, "")
    }
  }
}
