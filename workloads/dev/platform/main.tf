...
#
# Kubernetes pulls images from here. An image that is absent produces ImagePullBackOff,
# which reads as a broken deployment rather than an empty registry — so these exist before
# anything references them.
#
# ALL THREE ARE IMMUTABLE, and that is the fix for a defect this repository already had.
...
# first push succeeded and every one after it failed. The tools/* repositories had the
# mirror-image problem — MUTABLE with :latest, so nothing recorded which image was running.
#
# The resolution is not a middle setting. Nothing deploys :latest; every reference is a
# commit SHA or a digest. That is Property 5, and IMMUTABLE is what enforces it rather
# than documenting it.
...
module "ecr" {
  ...
  # force_delete = true, against the module's own default.
  #
  # This environment is torn down and rebuilt deliberately, and `terraform destroy` on a
  # repository holding images fails when force_delete is false — leaving the rest of the
  # layer standing and billing while the destroy sits half-done. Images here are
  # rebuildable from source, so discarding them is an inconvenience rather than a loss.
  #
  # Set false for anything holding an artefact that cannot be rebuilt.
  force_delete = true

  # Basic scanning, free and per-repository. Enhanced scanning via Inspector is a
  # registry-wide account setting and not this layer's business.
  scan_on_push = true

  # An immutable repository grows without bound: every build adds a tag that is never
  # replaced. Both rules are needed — the untagged one alone would never fire here,
  # because immutable tags are never orphaned by a re-push.
  untagged_image_expiry_days = 7
  max_tagged_images          = 30

  # No expirable_tag_prefixes: every image in a dev registry is disposable, so the count
  # applies to all of them. Reach for it when a tag must outlive the count.
}
